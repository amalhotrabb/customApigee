...
if Directory != None:
    ...
    # print repr(dirList)
    # print [dirEntry for dirEntry in dirList]
    # sys.exit(1)

    # for dir1, dir2, dir3 in os.walk(Directory):
    #     print dir1
    #     print dir2
    #     print dir3
    #     sys.exit(1)
	...
	exit()
#add copy statement for policcies & targets
        if not pathContainsDot(dirEntry[0]):
            for fileEntry in dirEntry[2]:
                #print fileEntry.endswith('~')
                if not fileEntry.endswith('~'):
                    # print dirEntry[0]+' asdfsadfs '+fileEntry
                    # sys.exit(1)

                    path1 = os.path.split(dirEntry[0])
 
                    if path1[1] != 'policies' and path1[1] != 'targets':
    ...
    dirPoliciesList = os.walk('../global/policies')
    realPath = 'apiproxy/policies'
    for dirEntry in dirPoliciesList:  
        if not pathContainsDot(dirEntry[0]):
            for fileEntry in dirEntry[2]:
                #print fileEntry.endswith('~')
                if not fileEntry.endswith('~'):
                    ...
                    en = os.path.join(realPath, fileEntry)
                    print 'Writing %s to %s' % (fn, en)
                    zipout.write(fn, en)

    # dirTargetList = os.walk('../global/targets')
    # realPath = 'apiproxy/targets'
    # fn = os.path.join(dirEntry[0], fileEntry)
    # en = os.path.join(realPath, fileEntry)
    # print 'Writing %s to %s' % (fn, en)
    # zipout.write(fn, en)
...
hdrs = {'Content-Type': 'application/octet-stream',
        'Accept': 'application/json', 'Api-Version': '2', 'Access-Control-Expose-Headers': 'Api-Version' }
uri = 'https://api.enterprise.apigee.com/v1/organizations/%s/apis?action=import&name=%s' % \
            (Organization, Name)
...
if ShouldDeploy:
    ...
    # Deploy the bundle
    hdrs = {'Accept': 'application/json', 'Api-Version': '2', 'Access-Control-Expose-Headers': 'Api-Version'}
    resp = httpCall('POST',
        ('/v1/organizations/%s/apis/%s/deployments' +
                '?action=deploy' +
                '&env=%s' +
